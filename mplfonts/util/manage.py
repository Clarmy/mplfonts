...
def use_font(font='Noto Sans CJK SC'):
    ...
    Args:
        font (str, optional): The font name that you want to use. Defaults to 'Noto Sans CJK SC'.
    """
...
def update_custom_rc(custom_rc=None):
    ...
    Args:
        custom_rc (str, optional): The file path of matplotlibrc. Defaults to None.
    """
    install_fonts()
    if not custom_rc:
    ...
    if os.path.exists(cache_rc_fp):
        ...
        if os.path.exists(backup_fp):
            ...
            print(
                f'Warning: The {matplotlib.matplotlib_fname()} already exists, renamed it to {backup_fp}'
            )
...
def install_fonts(font_dir=None):
    ...
    Args:
        font_dir (str, optional): The directory that storing font files. Defaults to None.
    """
    if not font_dir:
        font_dir = FONT_DIR
    ...
    for src_font_fp in font_fps:
        mpl_font_fp = os.path.join(
            MPL_FONT_PATH, os.path.basename(src_font_fp))
